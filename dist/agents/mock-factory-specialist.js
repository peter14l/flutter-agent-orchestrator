...
export class FlutterMockFactorySpecialistAgent extends BaseFlutterAgent {
    constructor() {
        super("FlutterMockFactorySpecialistAgent", "Demo Data & Mock Factory Specialist", "Generates realistic, domain-rich mock data factories, fake entities, and in-memory mock repositories for instant hackathon UI prototyping.");
    }
    generateFactory(spec) {
...
${Object.entries(spec.fields).map(([k, type]) => {
            if (type === "price" || type === "number")
                return `  final double ${k};`;
            if (type === "boolean")
                return `  final bool ${k};`;
            if (type === "date")
                return `  final DateTime ${k};`;
...
${Object.entries(spec.fields).map(([k, type]) => {
            if (type === "name")
                return `      ${k}: _names[index % _names.length],`;
            if (type === "email")
                return `      ${k}: 'user\${index + 1}@example.com',`;
            if (type === "price")
                return `      ${k}: (15.0 + (_random.nextDouble() * 250)).roundToDouble(),`;
            if (type === "number")
                return `      ${k}: (_random.nextInt(100) + 1).toDouble(),`;
            if (type === "date")
                return `      ${k}: DateTime.now().subtract(Duration(days: index * 2, hours: index * 3)),`;
            if (type === "status")
                return `      ${k}: _statuses[index % _statuses.length],`;
            if (type === "avatar")
                return `      ${k}: 'https://i.pravatar.cc/150?img=\${(index % 70) + 1}',`;
            if (type === "boolean")
                return `      ${k}: index % 2 == 0,`;
            if (type === "uuid")
                return `      ${k}: 'id-\${index + 1000}',`;
...
//# sourceMappingURL=mock-factory-specialist.js.map