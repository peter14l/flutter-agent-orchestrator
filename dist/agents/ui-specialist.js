...
export class FlutterUISpecialistAgent extends BaseFlutterAgent {
    constructor() {
        super("FlutterUISpecialistAgent", "Flutter UI/UX Design System & Widget Architect", "Designs production-grade, highly polished Flutter UI screens tailored to app archetypes (Fintech, E-Commerce, Fitness, AI Chat, Dashboards) with clean Material 3 standards, 8-pt spatial grid, responsive LayoutBuilder, and fluid micro-animations.");
    }
    detectArchetype(description, screenName) {
        const text = `${description} ${screenName}`.toLowerCase();
        if (text.match(/wallet|crypto|expense|budget|bank|finance|payment|transaction|invoice/))
            return "fintech";
        if (text.match(/shop|store|cart|product|ecommerce|order|catalog|food|restaurant|delivery/))
            return "ecommerce";
        if (text.match(/fit|workout|exercise|gym|calorie|health|running|step|activity/))
            return "fitness";
        if (text.match(/ai|chat|bot|assistant|message|copilot|prompt|conversation/))
            return "ai_chat";
        if (text.match(/dashboard|metric|analytics|stats|chart|kpi|saas|admin|report/))
            return "dashboard";
        if (text.match(/social|post|feed|story|profile|community|photo|video|follow/))
            return "social";
        return "general";
    }
    hasGlassmorphism(description) {
...
5. **State & Performance**: Reactive \`ConsumerWidget\` bound to auto-disposed Riverpod providers with 100% const constructors on static subtrees.
    ...
    generateDomainModel(archetype, baseName, snakeName) {
...
`;
    ...
    generateStateController(archetype, baseName, snakeName, controllerName, stateName) {
...
`;
    ...
    generateMockData(archetype, baseName) {
        if (archetype === "fintech") {
            return `
      final transactions = [
    ...
    generateWidgetScreen(archetype, useGlass, baseName, snakeName, screenClassName, stateClassName, controllerClassName) {
