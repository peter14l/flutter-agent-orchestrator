...
export class FlutterPromptDecomposerAgent extends BaseFlutterAgent {
    constructor() {
        super("FlutterPromptArchitectAgent", "Lead Flutter Solutions Architect & Requirements Decomposer", "Deconstructs Flutter project prompts, validates target platforms (iOS, Android, Web, Desktop), and builds Clean Architecture feature blueprints.");
    }
    detectPlatforms(prompt, explicitPlatforms) {
        ...
        if (p.includes("ios") || p.includes("iphone") || p.includes("ipad") || p.includes("apple"))
            detected.push("ios");
        if (p.includes("web") || p.includes("browser") || p.includes("wasm"))
            detected.push("web");
        if (p.includes("macos") || p.includes("mac os"))
            detected.push("macos");
        if (p.includes("windows") || p.includes("win32") || p.includes("win64"))
            detected.push("windows");
        if (p.includes("linux") || p.includes("ubuntu"))
            detected.push("linux");
        if (p.includes("desktop")) {
            if (!detected.includes("macos"))
                detected.push("macos");
            if (!detected.includes("windows"))
                detected.push("windows");
            if (!detected.includes("linux"))
                detected.push("linux");
        }
        if (p.includes("mobile")) {
            if (!detected.includes("android"))
                detected.push("android");
            if (!detected.includes("ios"))
                detected.push("ios");
        }
...
Please reply with your target platform choice to generate platform-specific configurations, permissions, and responsive layouts.`,
    ...
    decompose(prompt, targetPlatforms, projectName = "flutter_app", stateManagement = "riverpod", architecturePattern = "feature-first-clean-architecture") {
...
//# sourceMappingURL=prompt-decomposer.js.map