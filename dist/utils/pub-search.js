export const FLUTTER_ECOSYSTEM_REGISTRY = {
    // State Management
...
export class PubSearchHelper {
    static async searchPubDev(query) {
        try {
        ...
        return Object.values(FLUTTER_ECOSYSTEM_REGISTRY).filter(item => item.name.includes(qLower) || item.category.includes(qLower) || item.description.toLowerCase().includes(qLower)).slice(0, 5);
    }
    static generatePubspec(projectName = "flutter_app", targetPlatforms = ["android", "ios"], stateManagement = "riverpod", extraPackages = []) {
        const dependencies = {
            flutter: "sdk: flutter",
            go_router: "^14.8.0",
        ...
        for (const pkg of extraPackages) {
            ...
            if (pkg.includes("supabase"))
                dependencies["supabase_flutter"] = "^2.8.4";
        ...
        pubspecLines.push(``, `flutter:`, `  uses-material-design: true`, `  assets:`, `    - assets/images/`, `    - assets/icons/`);
...
//# sourceMappingURL=pub-search.js.map