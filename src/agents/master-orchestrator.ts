...
export class FlutterMasterOrchestratorAgent extends BaseFlutterAgent {
  private decomposer: FlutterPromptDecomposerAgent;
  private researcher: FlutterDependencyResearcherAgent;
  private uiSpecialist: FlutterUISpecialistAgent;
  private backendSpecialist: FlutterBackendSpecialistAgent;
  private tester: FlutterTestingSpecialistAgent;

  constructor() {
    super(
      "FlutterMasterOrchestratorAgent",
      "Lead Flutter Multi-Agent Pipeline Coordinator",
      "Coordinates all specialized Flutter agents, checks target platform requirements, and generates complete Flutter project scaffolds."
    );
  ...
  public async orchestrateProject(spec: FullFlutterPipelineSpec): Promise<FullFlutterPipelineResult> {
    ...
    const architecture = this.decomposer.decompose(
      spec.prompt,
      spec.targetPlatforms,
      projectName,
      spec.stateManagement || "riverpod"
    );
    ...
    const dependencyResult = await this.researcher.researchAndGeneratePubspec(
      projectName,
      targetPlatforms,
      spec.stateManagement || "riverpod"
    );
