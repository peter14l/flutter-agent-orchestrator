...
import { BackendAndStateSpec, BackendAndStateResult } from "../types.js";

export class FlutterBackendSpecialistAgent extends BaseFlutterAgent {
  constructor() {
    super(
      "FlutterBackendAndStateSpecialistAgent",
      "Flutter Clean Architecture & State Engineer",
  ...
  public scaffoldBackend(spec: BackendAndStateSpec): BackendAndStateResult {
...
function entityNameModelMapping(name: string): string {
  return `${name}Model({
    required super.id,
    required super.name,
