...
export class FlutterDatabaseSpecialistAgent extends BaseFlutterAgent {
  constructor() {
    super(
      "FlutterDatabaseSpecialistAgent",
      "Offline-First Database Architect & Migration Engineer",
      "Scaffolds type-safe Drift (SQLite) database schemas, DAOs with reactive streams, schema migrations, and local storage integration."
    );
  }

  public scaffoldDatabase(spec: FlutterDatabaseSpec): FlutterDatabaseResult {
...
${Object.entries(t.columns).map(([col, type]) => {
  if (type === "integer") return `  IntColumn get ${col} => integer()${t.primaryKey === col ? ".autoIncrement()" : "()"}();`;
  if (type === "real") return `  RealColumn get ${col} => real()();`;
  if (type === "boolean") return `  BoolColumn get ${col} => boolean().withDefault(const Constant(false))();`;
  if (type === "datetime") return `  DateTimeColumn get ${col} => dateTime()();`;
